<template>
    ...
    <div>
        <h1 class="text-7xl font-Inter font-bold text-secondary text-center mt-12 mb-16">Eventos</h1>
        <div class="overflow-x-auto">
            <table class="table table-lg w-3/4 mx-auto">
                <thead>
                    <tr>
                        <th class="text-2xl">Name</th>
                        <th class="text-2xl">Data</th>
                        <th class="text-2xl">Ingressos</th>
                        <th class="text-2xl">Ação</th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="evento in eventos" :key="evento.id">
                        <td class="text-2xl">{{ evento.name }}</td>
                        <td class="text-2xl">{{ formatarDataBrasileira(evento.date) }}</td>
                        <td class="text-2xl">{{ evento.totalTickets }}</td>
                        <td>
                            <button @click="abrirModalCompra(evento)" class="btn">Comprar</button>
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        <!-- Modal de Compra -->
        <div v-if="modalAberto" @click.away="fecharModalCompra" class="fixed inset-0 overflow-y-auto">
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div
                    class="inline-block align-bottom bg-base-300 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:w-full sm:max-w-lg">
                    <div class="bg-base-300 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="sm:flex sm:items-start">
                            <div
                                class="mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-base-100 sm:mx-0 sm:h-10 sm:w-10">
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                ...
                                <div class="mt-2">
                                    <label class="label" for="quantidadeIngressos">Quantidade de Ingressos:</label>
                                    <input type="number" v-model="quantidadeIngressos" class="input">
                                </div>
                                <div class="mt-2">
                                    <label class="label" for="metodoPagamento">Método de Pagamento:</label>
                                    <select v-model="metodoPagamento" class="select">
                    ...
                    <div class="bg-base-300 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                        <button @click="efetuarCompra" type="button"
                            class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-green-600 text-base font-medium text-white hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 sm:ml-3 sm:w-auto sm:text-sm">
                            Comprar
                        </button>
                        <button @click="fecharModalCompra" type="button"
                            class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-secondary text-base font-medium text-white hover:bg-secondary-content focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">
                            Cancelar
                        </button>
                    </div>
                </div>
...
export default {
    ...
    methods: {
        ...
        efetuarCompra() {
            alert('Compra em andamento!');
            const chavePix = this.gerarChavePixAleatoria();
            alert(`Efetue o pagamento via PIX para a chave: ${chavePix}`);
            this.resetarCompra();
        ...
        gerarChavePixAleatoria() {
            ...
            for (let i = 0; i < 20; i++) {
                ...
                chavePix += caracteres.charAt(randomIndex);
            }
            return chavePix;
        },
        resetarCompra() {
    ...
    mounted() {
        const eventoJSON = localStorage.getItem('evento');
        if (eventoJSON) {
            this.eventos = [JSON.parse(eventoJSON)];
        }
    },
};
</script>